...
TEST_CASE("test_init_clusters", "[init]"){
    ...
    //}
    PPF<double> first_cluster = ppfs[random_cluster_index];
    ...
    double sum = 0.0;
    double distance{};
    std::vector<double> distances(pdfs.size());
    std::vector<double> prob(pdfs.size());
    ...
    PPF<double> second_cluster = ppfs[random_cluster_index];
    ...
    PPF<double> third_cluster = ppfs[random_cluster_index];
...
TEST_CASE("test_init_bounds", "[init]"){
    PPF_SIZE = 500;
    build_data();
    WKmeans<double> test_wkmeans(pdfs.size(), N_CLUSTERS, EPSILON, pdfs, cdfs, ppfs, 10);
    test_wkmeans.init_clusters();
    ...
    std::vector<double> test_upper_bounds(pdfs.size(), std::numeric_limits<double>::infinity());
    std::vector<double> test_lower_bounds(pdfs.size() * N_CLUSTERS);
    std::vector<int> cluster_assignments(pdfs.size());
    for(int i = 0; i < pdfs.size(); i++){
        for(int k = 0; k < N_CLUSTERS; k ++){
            double distance = wasserstein_2(ppfs[i], test_wkmeans.clusters[k]);
    ...
    for(int i = 0; i < pdfs.size(); i++){
        for(int k = 0; k < N_CLUSTERS; k++){
            double d =  wasserstein_2(ppfs[i], test_wkmeans.clusters[k]);
